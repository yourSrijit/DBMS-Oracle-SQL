...
- Add new column
 ...
 ALTER TABLE STUDENT ADD (EID VARCHAR2(2 ),REGNO NUMBER(5));
 ```
- Modify
 ```
...
-RENAME
```
ALTER TABLE STUDENT RENAME COLUMN REGNO TO REGINO;

ALTER TABLE STUENT RENAME TO STUDENTS;   -> CHANGE TABLE RENAME
```
- Add Primary Key
 ```
...
### HAVING KEYWORD
- Having clause restricting the groups;
- It eleminate the non-matching groups;
...
# Day 5
## INTERSECT UNIONL MINUS 
```
  ((SELECT NAME FROM STUDENT WHERE ID IN ('E01','E06')) INTERSECT (SELECT NAME FROM STUDENT WHERE ID IN('E06','E07')));

  ((SELECT NAME FROM STUDENT WHERE ID IN ('E01','E06')) MINUS (SELECT NAME FROM STUDENT WHERE ID IN('E06','E07')));
 ...
 ---
 ###  GRANT
 first create a new user like this
 ```
  1. create user srijit identified by system;  //<- password //
 ```
 Try to connect in new tab 
 ```
 ERROR:
ORA-01045: user SRIJIT lacks CREATE SESSION privilege; logon denied  //give this kind of error


 grant connect to srijit;  // run on the 1st tab to grant access of the new user to connect only
 grant select on student to srijit;   //give asscess of the table only
 grant delete on student to srijit;   //give asscess to delete the table only
 grant all on student to srijit;   //give all asscess of the table to the srijit user
...
<!-- Grant succeeded.  -->
 ...
 ### REVOKE
 ```
 REVOKE DELETE ON SYSTEM.STUDENT FROM SRIJIT;
...
### Print the 4th ROW in Table 
```
 select r.* from (select s.*,ROWNUM rno from student s) r where rno=4;
```